...
angular
    ...
    .config(['SirTrevorProvider', 'config', function(SirTrevor, config) {
        ...
        SirTrevor.Blocks.Embed = SirTrevor.Block.extend({
            ...
            renderCard: function(data) {
                ...
                if (!_.has(data, 'html') && !_.isEmpty(data.thumbnail_url)) {
                    ...
                    var coverWidth = Math.min(this.getOptions().coverMaxWidth, data.thumbnail_width);
                    var coverHeight = coverWidth / ratio;
                ...
                if (_.has(data, 'provider_name')) {
                    var creditText = data.provider_name;
            ...
            loadData: function(dataParam) {
                ...
                var $coverHandler = this.$('.cover-preview-handler');

                if ($coverHandler.length > 0 && !$coverHandler.hasClass('hidden')) {
                    var $coverPreview = $coverHandler.find('.cover-preview');
                    var $removeLink = $('<a href="#">').text('hide the illustration');
                    var $showLink = $('<a href="#">')
        ...
        SirTrevor.DEFAULTS.Block.uploadOptions = uploadOptions;
        ...
        var Link = SirTrevor.Formatter.extend({
            ...
            onClick: function() {
                var selectionText = document.getSelection(),
                    ...
                    linkRegex = /((ftp|http|https):\/\/.)|mailto(?=\:[-\.\w]+@)/;
